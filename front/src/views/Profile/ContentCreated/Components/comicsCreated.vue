<template>
    <div class="container" v-if="comics != ''">
            ...
            <div class="row" v-for="comicsChunk in paginatedItems" :key="comicsChunk[0]._id">
                <div class="comics" v-for="comic in comicsChunk" :key="comic._id">
                    ...
                    <div class="show-options">
                        ...
                        <div style="margin:5px">
                            <button @click="openComic(comic._id)" class="btn-options"><v-icon name="hi-eye" color="white" scale="1"/></button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div style="margin: 15px">
            <button class="button-next" @click="prevPage" :disabled="currentPage === 1">«</button>
            <span style="margin-inline: 5px;">Página {{ currentPage }} de {{ totalPages }}</span>
            <button class="button-next" @click="nextPage" :disabled="currentPage === totalPages">»</button>
...
const totalPages = computed(() => Math.ceil(comicsChunks.value.length / itemsPerPage.value));
...
.show-options {
    position: absolute;
    background-image: linear-gradient(to bottom right, rgba(0, 0, 0, 0.5), rgba(56, 51, 51, 0.5));
    color: azure;
    z-index: 1;
    display: none;
    align-items: center;
