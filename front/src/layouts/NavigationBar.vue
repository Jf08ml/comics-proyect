    ...
    <div class="menu-hamburger">
      ...
      <div class="menu" :class="{ 'menu-open': isOpen }">
        <router-link custom to="/realcomics">
          <template v-slot="{ navigate }">
            <button @click="navigate" style="width: 40%; font-size: 20px; color: #b81f59;"><v-icon name="fa-person-booth"
                scale="1.2" title="Real Comics" color="#b81f59" /> Real</button>
          </template>
        </router-link>
        <router-link custom to="/animatedcomics">
          <template v-slot="{ navigate }">
            <button @click="navigate" style="width: 100%;font-size: 20px; color: #b81f59"><v-icon name="si-gitbook"
                scale="1.5" title="Real Comics" color="#b81f59" /> Animated</button>
          </template>
        </router-link>
        <router-link custom to="/login">
          <template v-slot="{ navigate }">
            <button @click="navigate" style="width: 100%;font-size: 20px; color: #b81f59"><v-icon name="md-login"
                scale="1.2" title="Real Comics" color="#b81f59" /> Log In</button>
          </template>
        </router-link>
        <router-link custom to="/signup">
          <template v-slot="{ navigate }">
            <button @click="navigate" style="width: 100%;font-size: 20px; color: #b81f59"><v-icon
