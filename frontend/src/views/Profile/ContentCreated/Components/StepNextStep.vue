    ...
    <div class="modal-body">
      <div class="modal-step" v-if="props.currentStep === 0">
        <div class="step-content">
          ...
          <p style="background-color: beige" v-if="props.addComicPart._id">
            You are adding a part to the following comic:
            <b>{{ props.addComicPart.nameSerie }}</b>
          </p>
          <div class="step">
      ...
      <div class="modal-step" v-if="props.currentStep === 1">
        <div class="step-content">
          ...
          <div class="step">
            <StepTwo :saveInfo="saveInfo" :infoComic="props.addComicPart" />
          </div>
        </div>
        <div class="change-step">
          <div class="content-btn-change">
            <button class="btn-change" @click="props.prevStep">
              <v-icon
                name="fa-arrow-left"
                scale="2"
                title="Previous"
        ...
        <div class="step-content">
          ...
          <div class="step">
            <StepThree
              ...
              :postInfoSaved="postInfoSaved"
            />
            <div class="loading-bar" v-if="isLoading"></div>
            <span v-if="isLoading"
              >The content is being uploaded, the waiting time will depend on
              the number of images, please wait</span
            >
          </div>
        </div>
        <div class="change-step">
          ...
          <div class="content-btn-change">
            <button @click="submitPost" class="btn-change-save">
              <v-icon name="io-send" scale="2" title="Save" color="green" />
            </button>
          </div>
        </div>
      </div>
